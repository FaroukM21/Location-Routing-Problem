...
class tabu_LRP():
    ...
    def cost_delivery(self,delivery):
        ...
        for i in range(len(sequence)-1):
            ...
            if( time < self.data[sequence[i],0]):
                twCost += self.OperationalCost * (self.data[sequence[i],0]-time/24)
            elif ( time > self.data[sequence[i],1]):
    ...
    def remove_depot(self, feasible_sol):
        ...
        if(len(depots)>=2):
            ...
            for i in indexes:
                ...
                """
                l = [j for j in range(len(sol)) if (sol[j]['depot'] == nearest_depot) 
                                            and (sol[j]['day']==sol[i]['day'])
                                            and (sol[j]['vehicle']==sol[i]['vehicle'])]
                
                if(len(l) == 2):
                    if(len(set([sol[j]['vehicle'] for j in l])) == 1 ) and (sol[i]['vehicle'] == 1):
                        sol[i]['vehicle'] = 2
                    elif (len(set([sol[j]['vehicle'] for j in l])) == 1 ) and (sol[i]['vehicle'] == 2):
                        sol[i]['vehicle'] = 1
                    l = [sol[j] for j in range(len(sol)) if (sol[j]['depot'] == nearest_depot) 
                                            and (sol[j]['day']==sol[i]['day'])]
                    print(l)"""
    ...
    def eps_greedy_policy(self, eps , Q, action):
    ...
    def generate_neighborhood_Q(self, sol, Q):
        nb_neigh = self.nb_neigh
        eps = 0.3
        gamma = 0.4
        alpha = 0.8
        ...
        #Q = np.zeros((10,10))
        nb_functions = 10 # number of neighbor generation function ( add_depot, remove_depot ...)
        action = self.eps_greedy_policy(eps, Q, None)
        for i in range(nb_neigh):
            eps = eps * 0.9
            neigh, reward = self.take_action(action, sol)
            new_action = self.eps_greedy_policy(eps , Q, action)
            Q[action, new_action] += alpha * (reward + gamma *np.max(Q[new_action]) - Q[action, new_action])
            action = new_action
            neighborhood.append(neigh)
    ...
    def solve(self, max_iter=100):
        ...
        for iteration in range(max_iter):
            ...
            depots = [element['depot'] for element in current_sol]
            ...
            neighborhood, Q = self.generate_neighborhood_Q(current_sol, Q)
...
if __name__ =="__main__":
    ...
    best_sol, Costs = tabu.solve(max_iter = 100)
    plt.plot(Costs)
